"""Backfill de mayo 2026 para días degradados por falla de Paso A.

Uso:
  python -m src.backfill_may_degraded_2026

Este script descarga todos los items disponibles en los feeds activos,
filtra por las fechas degradadas de mayo 2026, agrupa por fecha y ejecuta
run_pipeline con preloaded_articles para cada día, sobrescribiendo
data.json e index.html.
"""
...
def _fetch_may_2026_degraded_articles(sources: List[Dict]) -> Dict[str, List[Dict]]:
...
def run_backfill_may_degraded() -> int:
    """Ejecuta backfill para fechas degradadas de mayo 2026."""
    sources = _load_sources()
    by_date = _fetch_may_2026_degraded_articles(sources)
    target_dates = sorted(by_date.keys())

    print(f"Fechas objetivo mayo 2026 con articulos: {len(target_dates)}")
    if not target_dates:
        print("No hay articulos de mayo 2026 para backfill degradado.")
        return 1
    ...
    print("\n=== RESUMEN BACKFILL MAYO DEGRADADO ===")
...
if __name__ == "__main__":
    raise SystemExit(run_backfill_may_degraded())
